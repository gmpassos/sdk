...
class _IndexAssembler {
  ...
  AnalysisDriverUnitIndexBuilder assemble(CompilationUnit unit) {
    ...
    List<_ElementInfo> elementInfoList = elementMap.values.toList(growable: false);
    ...
    return AnalysisDriverUnitIndexBuilder(
        ...
        elementUnits: elementInfoList.map((e) => e.unitId).toList(growable: false),
        elementNameUnitMemberIds:
            elementInfoList.map((e) => e.nameIdUnitMember.id).toList(growable: false),
        elementNameClassMemberIds:
            elementInfoList.map((e) => e.nameIdClassMember.id).toList(growable: false),
        elementNameParameterIds:
            elementInfoList.map((e) => e.nameIdParameter.id).toList(growable: false),
        usedElements: elementRelations.map((r) => r.elementInfo.id).toList(growable: false),
        usedElementKinds: elementRelations.map((r) => r.kind).toList(growable: false),
        usedElementOffsets: elementRelations.map((r) => r.offset).toList(growable: false),
        usedElementLengths: elementRelations.map((r) => r.length).toList(growable: false),
        usedElementIsQualifiedFlags:
            elementRelations.map((r) => r.isQualified).toList(growable: false),
        usedNames: nameRelations.map((r) => r.nameInfo.id).toList(growable: false),
        usedNameKinds: nameRelations.map((r) => r.kind).toList(growable: false),
        usedNameOffsets: nameRelations.map((r) => r.offset).toList(growable: false),
        ...
        subtypes: subtypes.map((subtype) {
          return AnalysisDriverSubtypeBuilder(
            ...
            members: subtype.members.map((member) => member.id).toList(growable: false),
          );
        }).toList());
  }
